...
- `[x]` **Motor de Matching Inteligente (Inteligência Artificial & NLP):**
  - **O quê:** Substituir o matching de termos exatos por busca semântica inteligente.
  - **Como:** Motor adaptativo que consome a API da OpenAI/Gemini se chaves de API estiverem ativas, ou faz fallback local usando a distância Jaro-Winkler de similaridade de strings associando termos correlacionados e equivalências tecnológicas (ex: React = ReactJS, Postgres = PostgreSQL). Justificativa de IA exibida nos cards da shortlist.
